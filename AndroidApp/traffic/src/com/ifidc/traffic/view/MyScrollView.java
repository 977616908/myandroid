...
import com.ifidc.traffic.util.ImageLoader;
import com.ifidc.traffic.util.Images;
...
//			thirdColumn = (LinearLayout) findViewById(R.id.third_column);
	...
	public void checkVisibility() {
		for (int i = 0; i < imageViewList.size(); i++) {
			View views = imageViewList.get(i);
			ImageView view=((ImageView)((ViewGroup)views).getChildAt(0));
	...
	class LoadImageTask extends AsyncTask<String, Void, Bitmap> {
		...
		private void addImage(Bitmap bitmap, int imageWidth, int imageHeight) {
			...
			if (view != null) {
				((ImageView)((ViewGroup)view).getChildAt(0)).setImageBitmap(bitmap);
			} else {
				...
				tvView.setText("aaaa");
				lin.addView(tvView);
				findColumnToAdd(lin, imageHeight).addView(lin);
				imageViewList.add(lin);
			}
		}
		 ...
		 * @return 应该添加图片的一列
		 */
		private LinearLayout findColumnToAdd(View views,
				int imageHeight) {
			ImageView view=((ImageView)((ViewGroup)views).getChildAt(0));
